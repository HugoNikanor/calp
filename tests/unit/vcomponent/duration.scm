...
;;; TODO these tests don't belong to vcomponent anymore, but to datetime

(test-group "Parse duration"
 (test-equal (duration day: 15 hour: 5 second: 20)
 ...
 (test-equal (duration sign: '- minute: 15)
   (string->duration "-PT15M"))
 (test-equal (duration minute: 15)
   (string->duration "PT15M"))
 (test-equal (duration hour: 1)
   (string->duration "PT1H"))
 (test-equal (duration hour: 1)
   (string->duration "PT1H0M0S"))
 (test-equal (duration sign: '- minute: 30)
   (string->duration "-PT30M"))
 (test-equal (duration hour: 3)
   (string->duration "PT3H"))
 (test-equal (duration hour: 5 minute: 30)
   (string->duration "PT5H30M"))
 (test-equal (duration minute: 5)
   (string->duration "PT5M"))
 (test-equal (duration hour: 6 minute: 30)
   (string->duration "PT6H30M"))
 (test-equal (duration hour: 8 minute: 30)
